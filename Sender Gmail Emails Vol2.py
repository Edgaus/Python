...
current_dir = os.getcwd() + '\\Datos\\Sender Gmail Emails\\Capitulo_test.xlsx'
...
def sender_html(person, filenames,i,pdf_names,mails):
    ...
    cc = 'sociedadmctsm.congreso@gmail.com'
    ...
    # Attach the body of the message
        ...
        with open('Datos\\Sender Gmail Emails\\index.html', 'r') as file:
            html_text = file.read()
...
<div class="img-container">
  ...
  <img src="https://scontent.fcul2-1.fna.fbcdn.net/v/t39.30808-6/600202295_872003592006680_1256470669035829883_n.jpg?_nc_cat=111&ccb=1-7&_nc_sid=127cfc&_nc_ohc=xvYzbEHpoT0Q7kNvwHoRt_k&_nc_oc=Adm6GZWrpeVTLBqtKNxCRQQ7EGFeyDkrP_zXlIMeXzh9ujH8F3QsErlIhVF-NtyQo-lywvLUKfdNA7N106njpvGk&_nc_zt=23&_nc_ht=scontent.fcul2-1.fna&_nc_gid=mHy8MFy0du__AOmp4JlI5g&oh=00_Afl4jILupT8v7TM6Zy2KsmPPuQt088jPufGyhYS9gtFRNA&oe=69469F0F" 
 ...
 >
  ...
  ">
   ...
   <table cellpadding="0" cellspacing="0" align="center">
     ...
     <span style="text-transform: uppercase; font-weight: bold; font-size: 17px;">   {Name}</span>
  ...
  <p style="
    ...
    color: #333333;
  ">
 La mesa directiva del Capítulo Estudiantil de la Sociedad Mexicana de Ciencia y Tecnología de Superficies y Materiales A.C. (SMCTSM) agradece tu participación con el trabajo titulado
  </p>
  ...
  <!-- Work title -->
   <table cellpadding="0" cellspacing="0" align="center">
     ...
     <span style="text-transform: uppercase; font-weight: bold; font-size: 17px;">   {Trabajo}</span>
  ...
  <p style="
    ...
    margin-top: 25px;
  ">
    La constancia de aceptación viene adjunta en este correo.
  </p>
...
for i in range(limit):
    ...
    version = 'Datos\\Sender Gmail Emails\\Constancia Capítulo Estudiantil SMCTSM.docx'
    doc = DocxTemplate(version)
    doc.render(extract)

    # Set the paths for the Word and PDF files
    word_path = f'Datos\\Sender Gmail Emails\\Documents_Generated\\Word\\Constancia Capítulo Estudiantil SMCTSM {Submission_Id}.docx'
    pdf_path = f'Datos\\Sender Gmail Emails\\Documents_Generated\\PDF\\Constancia Capítulo Estudiantil SMCTSM {Submission_Id}.pdf'
    pdf_name = f"Constancia Participacion Capitulo Estudiantil SMCTSM.pdf"
    ...
    version = 'Datos\\Sender Gmail Emails\\Constancia Asistencia Capítulo Estudiantil SMCTSM.docx'
    doc = DocxTemplate(version)
    doc.render(extract)


    # Set the paths for the Word and PDF files
    word_path_asistencia = f'Datos\\Sender Gmail Emails\\Documents_Generated Asistencia\\Word\\Constancia Asistencia Capítulo Estudiantil SMCTSM {Submission_Id}.docx'
    pdf_path_asistencia = f'Datos\\Sender Gmail Emails\\Documents_Generated Asistencia\\PDF\\Constancia Asistencia Capítulo Estudiantil SMCTSM {Submission_Id}.pdf'
    pdf_name_asistencia = f"Constancia Asistencia Capitulo Estudiantil SMCTSM.pdf"


    doc.save(word_path_asistencia)

 
    # Load the Word document using the docx library
    doc = docx.Document(word_path_asistencia)
 
    # Save the Word document as a PDF using Microsoft Word
    word = comtypes.client.CreateObject("Word.Application")
    docx_path_asistencia = os.path.abspath(word_path_asistencia)
    pdf_path_asistencia = os.path.abspath(pdf_path_asistencia)

 
    pdf_format = 17  # PDF file format code
    word.Visible = False
    in_file = word.Documents.Open(docx_path_asistencia)
    in_file.SaveAs(pdf_path_asistencia, FileFormat=pdf_format)
    in_file.Close()
    ...
    sender_html(extract,[pdf_path, pdf_path_asistencia],i,[pdf_name, pdf_name_asistencia],[authors_email])
